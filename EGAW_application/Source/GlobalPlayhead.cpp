...
void GlobalPlayhead::setSampleRate(double newSampleRate)
{
    ...
    DBG("GLOBAL PLAYHEAD SAMPLE RATE SET: " << newSampleRate);
}
...
void GlobalPlayhead::setBufferSize(double newBufferSize)
{
    ...
    DBG("GLOBAL PLAYHEAD BUFFER SIZE SET: " << newBufferSize);
}
...
void GlobalPlayhead::add_seconds(double seconds)
{
        auto newPosition = globalPlayhead.load() + (seconds * sampleRate);
		setPlayheadPosition(newPosition);
}

void GlobalPlayhead::substract_seconds(double seconds)
{
	auto newPosition = globalPlayhead.load() - (seconds * sampleRate);
    if(newPosition > 0)
	    setPlayheadPosition(newPosition);
