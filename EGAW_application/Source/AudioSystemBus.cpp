...
void AudioSystemBus::valueTreeChildRemoved(juce::ValueTree& parentTree, juce::ValueTree& childWhichHasBeenRemoved, int indexFromWhichChildWasRemoved)
{
    if (parentTree == projectData)
    {
		...
		else if (PluginRackChannel* pluginRackChannel = dynamic_cast<PluginRackChannel*>(channel))
		{
			removePluginRackChannel(pluginRackChannel, childWhichHasBeenRemoved.getChildWithName("channelsRoutedTo"));
		}
...
void AudioSystemBus::handleMessage(const juce::Message& message)
{
    if (auto* signalMsg = dynamic_cast<const SignalMessage*>(&message)) {
        ...
        case SignalManagerUI::Signal::ADD_MIX_BUS_CHANNEL:
            addMixBusChannel();
            break;
		case SignalManagerUI::Signal::ADD_PLUGIN_RACK_CHANNEL:
			addPluginRackChannel();
			break;
        default:
            break;
        }
...
void AudioSystemBus::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
	...
	for (auto* pluginRack : pluginRackChannels)
		pluginRack->prepareToPlay(samplesPerBlockExpected, sampleRate);
}
...
void AudioSystemBus::addPluginRackChannel()
{
    pluginRackChannels.add(new PluginRackChannel(globalPlayhead));
    pluginRackChannels.getLast()->setMixerPosition(channelOrder.getNumChildren());
    pluginRackChannels.getLast()->routeTo(&masterBusChannel, true);

    channelOrder.appendChild(juce::ValueTree(pluginRackChannels.getLast()->getInternalChannelId()), nullptr);
    auto pluginRackChannelTree = pluginRackChannels.getLast()->getSettings();

    auto nombrePluginRackChannel = juce::String("Plugin Rack ");
    nombrePluginRackChannel.append(juce::String(pluginRackChannels.size()), 3);
    pluginRackChannelTree.setProperty("Name", nombrePluginRackChannel, nullptr);

    projectData.appendChild(pluginRackChannelTree, nullptr);
...
void AudioSystemBus::removePluginRackChannel(PluginRackChannel* pluginRackChannelToRemove, juce::ValueTree channelsRoutedTo)
{
    if (pluginRackChannelToRemove->isSoloOn())
    {
        SoloControlSingleton::getInstance()->setSignal(SoloControlSingleton::SoloSignal::GLOBAL_SOLO_OFF);
    }
    ...
    for (int i = 0; i < channelsRoutedTo.getNumChildren(); ++i)
    {
        ...
        if (auto routableChannel = getRoutableChannelByUUID(routeChannelUuid))
        {
            pluginRackChannelToRemove->removeRouteTo(routableChannel);
        }
    }

    auto pluginRackChannelUuid = pluginRackChannelToRemove->getInternalChannelId();
    //Nos saltamos el primer hijo porque es channelOrder
    for (int i = 1; i < projectData.getNumChildren(); ++i)
    {
        auto channelData = projectData.getChild(i);
        ...
        for (int j = 0; j < targetChannelsRoutedTo.getNumChildren(); ++j)
        {
            if (pluginRackChannelUuid == targetChannelsRoutedTo.getChild(j).getType().toString())
            {
                auto channel = getChannelByUUID(channelData.getType().toString());
                channel->removeRouteTo(pluginRackChannelToRemove);
            }
        }
    }

    pluginRackChannels.removeObject(pluginRackChannelToRemove, true);
}

juce::Array<PluginRackChannel*> AudioSystemBus::getPluginRackChannels()
{
    juce::Array<PluginRackChannel*> pluginRackChannelsResult;
    for (PluginRackChannel* c : pluginRackChannels)
    {
        pluginRackChannelsResult.add(c);
    }
    return pluginRackChannelsResult;
}
...
void AudioSystemBus::restoreProjectData()
{
    ...
    for (int i = 0; i < channelOrderRestore.getNumChildren(); ++i)
    {
        ...
        else
        {
            ...
            else if (channelType == "PluginRackChannel")
            {
				pluginRackChannels.add(new PluginRackChannel(globalPlayhead, uuid, channelData));
				pluginRackChannels.getLast()->setMixerPosition(i);
				channelOrder.appendChild(juce::ValueTree(pluginRackChannels.getLast()->getInternalChannelId()), nullptr);
				projectData.appendChild(pluginRackChannels.getLast()->getSettings(), nullptr);
            }
        }
    }
    ...
    for (PluginRackChannel* pluginRackChannel : pluginRackChannels)
    {
        juce::ValueTree channelData = fileRestoreProjectData.getChildWithName(pluginRackChannel->getInternalChannelId());
        juce::ValueTree channelsRoutedTo = channelData.getChildWithName("channelsRoutedTo");
        ...
        for (int i = 0; i < channelsRoutedTo.getNumChildren(); ++i)
        {
            ...
            if (destinyChannel != nullptr)
                pluginRackChannel->routeTo(destinyChannel, true);
        }
    }
...
Channel* AudioSystemBus::getChannelByUUID(juce::String uuid)
{
    ...
    for (PluginRackChannel* channel : pluginRackChannels)
	{
		if (channel->getInternalChannelId() == uuid)
		{
			return channel;
