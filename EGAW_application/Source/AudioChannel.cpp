...
AudioChannel::AudioChannel(GlobalPlayhead& globalPlayhead) : globalPlayhead(globalPlayhead), settings(Channel::getInternalChannelId())
{
    ...
    settings.setProperty("Colour", colourConstants::audio_channel_base_colour.toString(), nullptr);
...
void AudioChannel::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
    ...
    */
    ...
    // Check if we need to process new audio
    if (!hasCachedBlock || cachedPosition != globalPosition)
    {
        // Clear buffer if we're outside playable region
        ...
        cachedBuffer.setSize(bufferToFill.buffer->getNumChannels(), bufferToFill.numSamples);
        juce::AudioSourceChannelInfo cacheInfo(&cachedBuffer, 0, bufferToFill.numSamples);

        // Get audio from source
        resampler->getNextAudioBlock(cacheInfo);

        // Apply processing
        juce::dsp::AudioBlock<float> block(cachedBuffer);
        if ((!channelMute) && ((!isGlobalSoloActive) || (isGlobalSoloActive && channelSolo)))
        {
            gain.process(juce::dsp::ProcessContextReplacing<float>(block));
            pan.process(juce::dsp::ProcessContextReplacing<float>(block));
    ...
    if (hasCachedBlock)
    {
        for (int ch = 0; ch < bufferToFill.buffer->getNumChannels(); ++ch)
        {
            bufferToFill.buffer->copyFrom(
                ...
                cachedBuffer,
                ch % cachedBuffer.getNumChannels(),
                0,
...
void AudioChannel::routeTo(RoutableChannel* channelToRoute, bool isNewRoute = false)
{
	Channel::routeTo(channelToRoute, true);
}

void AudioChannel::removeRouteTo(RoutableChannel* channelToRemoveRoute)
{
    Channel::removeRouteTo(channelToRemoveRoute);
}
