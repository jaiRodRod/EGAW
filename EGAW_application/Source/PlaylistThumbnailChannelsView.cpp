...
PlaylistThumbnailChannelsView::PlaylistThumbnailChannelsView(juce::ValueTree& projectData, juce::ValueTree& playheadState, 
    GlobalPlayhead& globalPlayhead, CustomScrollBar_LF& customScrollBar_LF)
...
void PlaylistThumbnailChannelsView::paint (juce::Graphics& g)
{
	...
	g.setColour(colourConstants::plain_green.darker(0.5f));
    g.fillRect(0.0f,0.0f,(float)getLocalBounds().getWidth(), channelOccupiedSpace);

    /*
    if (nChannels >= 10)
    {
        customScrollBar_LF.setLengthReached(true);
    }
    else
    {
		customScrollBar_LF.setLengthReached(false);
    }
    */
}
...
void PlaylistThumbnailChannelsView::setupFlexBoxLayout()
{
    ...
    if (auto* display = DisplaySingleton::getInstance())
    {
        ...
        for (auto* channel : audioThumbnailChannels)
        {
			...
			nChannels++;
			channelOccupiedSpace += channelHeight;
