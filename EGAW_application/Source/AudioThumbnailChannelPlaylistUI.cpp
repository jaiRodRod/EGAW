...
void AudioThumbnailChannelPlaylistUI::evaluateTimeLength()
{
	playheadState.setProperty("realAudioTime", 0, nullptr);

    auto channels = projectData.getChildWithName("channelOrder");
    for (int i = 0; i < channels.getNumChildren(); ++i)
    {
        auto channelUuid = channels.getChild(i).getType().toString();
        auto channel = projectData.getChildWithName(channelUuid);
        if (channel.getProperty("Type") == "AudioChannel")
        {
            globalPlayhead.contestForTimeLength((juce::int64)channel.getProperty("AudioFileEnd") * (double)playheadState.getProperty("sampleRate"));
        }
    }
}
